...
# def _test(opt, model, test_dates, IMAGE_SIZE, PATCH_SIZE):
#     cur_result = {}
#     model.eval()
#
...
#                         cur_result['ergas'] = 0
#
#                         for i in tqdm(range(6), desc="处理最后的图片"):
...
#         'dtype': np.int16
#     }
#     save_dir = '/home/zbl/codeLab/remotePython/SwinSTFM-main/paper_images'
#     print("保存输出结果成功！")
#     if not os.path.exists(save_dir):
#         os.makedirs(save_dir)
        ...
        if cur_day == 347:
            for ref_date in test_dates:
                ...
                if ref_day != cur_day:
                    ...
                    output_image = np.zeros(images[1].shape)
                    image_mask = np.ones(images[1].shape)
                    for i in tqdm(range(4), desc="Processing image"):
                        negtive_mask = np.where(images[i] < 0)
                        inf_mask = np.where(images[i] > 10000.)
                        image_mask[negtive_mask] = 0
                        image_mask[inf_mask] = 0

                    for i in tqdm(range(len(h_index_list)), desc="预测图像中"):
                        for j in range(len(w_index_list)):
                            h_start = h_index_list[i]
                            w_start = w_index_list[j]
                    ...
                    real_output = (output_image + 1) * 0.5 * image_mask
                    print("预测完成")
                    for real_predict in tqdm([real_output], desc='处理最终图像'):
                        cur_result['rmse'] = []
                        cur_result['ssim'] = []
                        cur_result['cc'] = []
                        cur_result['uiqi'] = []
                        ...
                        for i in tqdm(range(6), desc="计算损失"):
                            cur_result['rmse'].append(rmse(real_im[i], real_predict[i]))
                            cur_result['ssim'].append(ssim(real_im[i], real_predict[i], MAX=1.0)[0])
                            cur_result['uiqi'].append(uiqi(real_im[i], real_predict[i]))
                            cur_cc = np.sum(
                        ...
                        if ref_day != 363:
                            ...
                            save_dir = '/home/zbl/codeLab/remotePython/SwinSTFM-main/pre_image/'
                            if not os.path.exists(save_dir):
                                os.makedirs(save_dir)
                            im_name = os.path.join(save_dir, 'swin_LGC.tif')
                            assert final_im.ndim == 2 or final_im.ndim == 3
...
def train(opt, train_dates, test_dates, IMAGE_SIZE, PATCH_SIZE):
    ...
    model_CKPT = torch.load('/home/zbl/codeLab/remotePython/SwinSTFM-main/models/best/epoch_best.pth')
    pretained_dict = {k: v for k, v in model_CKPT.items() if k in G_dict}

    G_dict.update(pretained_dict)
    model_G.load_state_dict(G_dict)
    model_G.cuda()
    device_ids = [i for i in range(2)]
    model_G = nn.DataParallel(model_G, device_ids)
    _test(opt, model_G, test_dates, IMAGE_SIZE, PATCH_SIZE)
...
def main():
    ...
    # parser.add_argument('--num_epochs', default=8, type=int, help='train epoch number')
    # parser.add_argument('--root_dir', default='/home/zbl/datasets/STFusion/LGC/LGC_NPY/',
    #                     help='Datasets root directory')
    # parser.add_argument('--train_dir', default='/home/zbl/datasets/STFusion/LGC/LGC_Train/',
    #                     help='Datasets train directory')
    # parser.add_argument('--batch_size', default=10, type=int, help='number of images per batch')
    # parser.add_argument('--num_workers', default=12, type=int, help='Datasets train directory')

    parser = argparse.ArgumentParser(description='Train Super Resolution Models')
    parser.add_argument('--image_size', default=[2720, 3200], type=int, help='the image size (height, width)')
    parser.add_argument('--patch_size', default=256, type=int, help='training images crop size')
    parser.add_argument('--num_epochs', default=60, type=int, help='train epoch number')
    parser.add_argument('--root_dir', default='/home/zbl/datasets/STFusion/LGC/LGC_NPY/', help='Datasets root directory')
    parser.add_argument('--train_dir', default='/home/zbl/datasets/STFusion/LGC/LGC_Train/', help='Datasets train directory')
