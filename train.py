...
def train(opt, train_dates, test_dates, IMAGE_SIZE, PATCH_SIZE):
    ...
    best_ssim = 0.0
    best_epoch = -1
    save_dir = '/home/zbl/codeLab/remotePython/SwinSTFM-main/models/best'
    ...
    for epoch in range(opt.num_epochs):
        ...
        progress_bar = tqdm(enumerate(train_loader), total=batches)
        for item, (data, target, ref_lr, ref_target, gt_mask) in progress_bar:
            t_start = timer()
            ...
            if item % 200 == 199:
                print("\n")
                print(
                    '[%d/%d][%d/%d] G-Loss: %.4f Batch_Time: %.4f' % (
